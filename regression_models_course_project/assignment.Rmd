...
fit_multi <- lm(formula = mpg ~ am + cyl + disp + hp + wt, data = mtcars)
summary(fit_multi)
sqrt(vif(fit_multi))
```
...
As the ANOVA result shows, the 3rd model (only `am`, `wt` and `hp`) is perhaps our fit; the other models cannot justify their inclusion of additional regressors at a loss of our previous DFs. Therefore we conclude that **cars with manual transmission gets ~2 more miles per gallon than cars with automatic transmission**.